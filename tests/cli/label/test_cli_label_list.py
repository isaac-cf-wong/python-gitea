"""Unit tests for the label list command."""

from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from gitea.cli.label.list import list_command
...
def test_list_command_calls_execute_and_wires_params(mock_gitea, mock_get_auth_params, mock_execute):
    """list_command should lookup auth and pass an api_call that lists labels."""
    ctx = make_ctx()

    mock_get_auth_params.return_value = ("tok", "https://gitea.example.com")

    client = MagicMock()
    client.label.list_labels.return_value = ([{"id": 1, "name": "bug"}], {"status_code": 200})
    ...
    assert call_kwargs["command_name"] == "gitea-cli label list"

    result = call_kwargs["api_call"]()
    client.label.list_labels.assert_called_once_with(owner="owner", repository="repo", page=1, limit=20)
