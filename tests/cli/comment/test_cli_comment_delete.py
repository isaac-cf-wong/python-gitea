"""Unit tests for the comment delete command."""

from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from gitea.cli.comment.delete import delete_command
...
def test_delete_command_calls_execute_and_wires_params(mock_gitea, mock_get_auth_params, mock_execute):
    """delete_command should lookup auth and pass an api_call that deletes a comment."""
    ctx = make_ctx()

    mock_get_auth_params.return_value = ("tok", "https://gitea.example.com")

    client = MagicMock()
    client.comment.delete_comment.return_value = ({}, {"status_code": 204})
    ...
    assert call_kwargs["command_name"] == "gitea-cli comment delete"

    result = call_kwargs["api_call"]()
    client.comment.delete_comment.assert_called_once_with(owner="owner", repository="repo", comment_id=42)
