"""Unit tests for the comment list command."""

from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from gitea.cli.comment.list import list_command
...
def test_list_command_calls_execute_and_wires_params(mock_gitea, mock_get_auth_params, mock_execute):
    """list_command should lookup auth and pass an api_call that lists comments."""
    ctx = make_ctx()

    mock_get_auth_params.return_value = ("tok", "https://gitea.example.com")

    client = MagicMock()
    client.comment.list_comments.return_value = ([{"id": 1, "body": "Hello"}], {"status_code": 200})
    ...
    assert call_kwargs["command_name"] == "gitea-cli comment list"

    result = call_kwargs["api_call"]()
    client.comment.list_comments.assert_called_once_with(owner="owner", repository="repo", index=7, page=2, limit=10)
    assert result == ([{"id": 1, "body": "Hello"}], {"status_code": 200})
