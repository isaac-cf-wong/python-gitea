...
def delete_command(
    ...
    """Delete a label in a repository.
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        """
        target_repository = require_repository(repository, command="gitea-cli label delete")

        with Gitea(token=token, base_url=base_url) as client:
            return client.label.delete_label(
                owner=owner,
                repository=target_repository,
                label_id=label_id,
            )

    execute_api_command(api_call=api_call, base_url=base_url, command_name="gitea-cli label delete")
