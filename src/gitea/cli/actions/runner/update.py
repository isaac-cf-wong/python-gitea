...
from gitea.cli.actions.runner.state import RUNNER_STATE_HELP, RunnerState
from gitea.cli.utils.options import ADMIN_HELP, OWNER_SCOPE_HELP, REPOSITORY_SCOPE_HELP, RUNNER_ID_HELP

COMMAND_NAME = "gitea-cli actions runner update"


def update_runner_command(
    ctx: typer.Context,
    runner_id: Annotated[int, typer.Option("--runner-id", help=RUNNER_ID_HELP)],
    state: Annotated[RunnerState, typer.Option("--state", help=RUNNER_STATE_HELP)],
    ...
    Args:
        ...
        state: The state to put the runner in.
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        Returns:
            A tuple containing the runner as it now stands and metadata.

        """
        with reporting_scope_errors(COMMAND_NAME), Gitea(token=token, base_url=base_url) as client:
            return client.actions.update_runner(
                runner_id=runner_id,
                disabled=state is RunnerState.DISABLED,
                owner=owner,
