"""List the jobs of one Actions workflow run."""
...
COMMAND_NAME = "gitea-cli actions run jobs"

STATUS_HELP = "Status of the jobs to list: pending, queued, in_progress, failure, success or skipped."


def list_run_jobs_command(
    ctx: typer.Context,
    ...
    The `id` of a job here is what `gitea-cli actions job logs` takes: a run
    says which jobs it has, and the logs are read per job.
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        Returns:
            A tuple containing the job listing and metadata.

        """
        target_repository = require_repository(repository, command=COMMAND_NAME)

        with Gitea(token=token, base_url=base_url) as client:
            return client.actions.list_workflow_run_jobs(
                owner=owner,
