"""Mark an Actions workflow run cancelled without waiting for its jobs.

`gitea-cli actions run cancel` asks the jobs to stop and waits for them; a job
whose runner has died never answers, and the run stays `in_progress`. This marks
the run cancelled regardless, which is what clears a stuck run out of the way.

It is the blunter of the two on purpose: a job that is in fact still running
somewhere is not stopped by this, only disowned. Cancel first, and reach for this
when that has not taken effect.
"""
...
COMMAND_NAME = "gitea-cli actions run force-cancel"


def force_cancel_run_command(
    ctx: typer.Context,
