"""List a repository's Actions workflows."""
...
COMMAND_NAME = "gitea-cli actions workflow list"
...
def list_workflows_command(
    ...
    The `id` of each workflow is its file name, which is what `--workflow-id`
    takes everywhere else in this family.
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        Returns:
            A tuple containing the workflow listing and metadata.

        """
        target_repository = require_repository(repository, command=COMMAND_NAME)

        with Gitea(token=token, base_url=base_url) as client:
            return client.actions.list_workflows(owner=owner, repository=target_repository)
