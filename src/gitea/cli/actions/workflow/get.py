"""Get one Actions workflow of a repository."""
...
COMMAND_NAME = "gitea-cli actions workflow get"


def get_workflow_command(
    ctx: typer.Context,
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        Returns:
            A tuple containing the workflow and metadata.

        """
        target_repository = require_repository(repository, command=COMMAND_NAME)

        with Gitea(token=token, base_url=base_url) as client:
            return client.actions.get_workflow(
                owner=owner,
