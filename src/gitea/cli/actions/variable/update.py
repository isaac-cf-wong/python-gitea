"""Update an Actions variable of a repository, an organization or the account.

`--value` is required even by an update that only means to rename a variable:
Gitea requires the value in the body, so the value the variable is to keep has to
be sent again. Read it with `gitea-cli actions variable get` rather than guessing
it.

`--new-name` is the rename. Omitting it leaves the name alone.
...
COMMAND_NAME = "gitea-cli actions variable update"


def update_variable_command(
    ctx: typer.Context,
    ...
    new_name: Annotated[
        str | None, typer.Option("--new-name", help="Rename the variable to this. Omit it to leave the name alone.")
    ] = None,
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        with reporting_scope_errors(COMMAND_NAME), Gitea(token=token, base_url=base_url) as client:
            return client.actions.update_variable(
