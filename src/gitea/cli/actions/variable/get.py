"""Get one Actions variable of a repository, an organization or the account.

The value comes back under `data`, which is also what
`gitea-cli actions variable update` has to be given again: the endpoint requires
a value even when only the name is being changed, so reading it first is how a
rename keeps it.
...
COMMAND_NAME = "gitea-cli actions variable get"
...
def get_variable_command(
    ...
    def api_call() -> tuple[dict[str, Any] | list[dict[str, Any]], dict[str, Any]]:
        ...
        Returns:
            A tuple containing the variable, its value under `data`, and metadata.

        """
        with reporting_scope_errors(COMMAND_NAME), Gitea(token=token, base_url=base_url) as client:
            return client.actions.get_variable(
