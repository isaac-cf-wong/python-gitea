...
The asynchronous mirror of `gitea.actions.secret`. The endpoints, the arguments
and the answers are that module's, and it is the one to read for what each
method does and why. The difference here is `aiohttp` in place of `requests`,
and the awaits that come with it.
"""
...
class AsyncSecrets(BaseActions, AsyncResource):
    ...
    async def list_secrets(
        ...
        **kwargs: Any,
        ...
        """
        response = await self._list_secrets(owner=owner, repository=repository, page=page, limit=limit, **kwargs)
    ...
    async def create_or_update_secret(
        ...
        **kwargs: Any,
        ...
        """
        response = await self._create_or_update_secret(
            secret_name=secret_name,
            data=data,
            owner=owner,
            repository=repository,
        ...
        parsed, status_code = await process_async_response(response, default={})
    ...
    async def delete_secret(
        ...
        **kwargs: Any,
        ...
        """
        response = await self._delete_secret(secret_name=secret_name, owner=owner, repository=repository, **kwargs)
        data, status_code = await process_async_response(response, default={})
